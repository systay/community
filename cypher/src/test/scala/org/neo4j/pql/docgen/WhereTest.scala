...
class WhereTest extends DocumentingTestBase {
  ...
  @Test def filter_on_property() {
    testQuery(
      ...
      queryText = """select n from n=node(%Andres%, %Tobias%) where n.age < 30""",
      returns = """The node.""",
  ...
  @Test def boolean_operations() {
    testQuery(
      ...
      queryText = """select n from n=node(%Andres%, %Tobias%) where (n.age < 30 and n.name = "Tobias") or not(n.name = "Tobias") """,
      returns = """The node.""",
  ...
  @Test def regular_expressions() {
    testQuery(
      ...
      queryText = """select n from n=node(%Andres%, %Tobias%) where n.name =~ /Tob.*/""",
  ...
  @Test def has_property() {
    testQuery(
      ...
      queryText = """select n from n=node(%Andres%, %Tobias%) where n.belt""",
  ...
  @Test def compare_if_property_exists() {
    testQuery(
      ...
      queryText = """select n from n=node(%Andres%, %Tobias%) where n.belt? = 'white'""",
  ...
  @Test def filter_on_relationship_type() {
    testQuery(
      ...
      queryText = """select r from n=node(%Andres%) pattern (n)-[r]->() where type(r) =~ /K.*/""",
  ...
  @Test def filter_on_null() {
    testQuery(
      ...
      queryText = """select b from a=node(%Tobias%), b=node(%Andres%, %Peter%) pattern a<-[r?]-b where r is null""",
