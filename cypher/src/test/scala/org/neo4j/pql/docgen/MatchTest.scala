 ...
 */
package org.neo4j.pql.docgen

import org.junit.Test
import org.junit.Assert._
import org.neo4j.graphdb.{DynamicRelationshipType, Path, Node}
import org.neo4j.pql.CuteGraphDatabaseService.gds2cuteGds
...
class MatchTest extends DocumentingTestBase {
  ...
  @Test def allRelationships() {
    testQuery(
      ...
      queryText = """select x from n=node(%A%) pattern (n)--(x)""",
  ...
  @Test def allOutgoingRelationships2() {
    testQuery(
      ...
      queryText = """select r from n=node(%A%) pattern (n)-[r]->()""",
  ...
  @Test def relatedNodesByRelationshipType() {
    testQuery(
      ...
      queryText = """select x from n=node(%A%) pattern (n)-[:BLOCKS]->(x)""",
  ...
  @Test def relationshipsByType() {
    testQuery(
      ...
      queryText = """select r from n=node(%A%) pattern (n)-[r:BLOCKS]->()""",
  ...
  @Test def relationshipsByTypeWithSpace() {
    ...
    testQuery(
      ...
      queryText = """select r from n=node(%A%) pattern (n)-[r:`TYPE WITH SPACE IN IT`]->()""",
  ...
  @Test def multiStepRelationships() {
    testQuery(
      ...
      text = "Relationships can be expressed by using multiple statements in the form of `()--()`, or they can be strung together, " +
        "like this:",
      queryText = """select a,b,c from a=node(%A%) pattern (a)-[:KNOWS]->(b)-[:KNOWS]->(c)""",
  ...
  @Test def variableLengthPath() {
    testQuery(
      ...
      queryText = """select a,x from a=node(%A%), x=node(%E%, %B%) pattern a-[:KNOWS*1..3]->x""",
  ...
  @Test def zeroLengthPath() {
    testQuery(
      ...
      queryText = """select a,b,c, length(p1), length(p2) from a=node(%A%) pattern p1=a-[:KNOWS*0..1]->b, p2=b-[:BLOCKS*0..1]->c""",
  ...
  @Test def fixedLengthPath() {
    testQuery(
      ...
      queryText = """select p from a=node(%D%) pattern p=a-[*3]->()""",
  ...
  @Test def optionalRelationship() {
    testQuery(
      ...
      text = "If a relationship is optional, it can be marked with a question mark. This similar to how a SQL outer join " +
        ...
        "path.",
      queryText = """select a,x from a=node(%E%) pattern a-[?]->x""",
  ...
  @Test def nodePropertyFromOptionalNode() {
    testQuery(
      ...
      queryText = """select x, x.name from a=node(%E%) pattern a-[?]->x""",
  ...
  @Test def optionalTypedRelationship() {
    testQuery(
      ...
      text = "Just as with a normal relationship, you can decide which identifier it goes into, and what relationship type " +
        "you need.",
      queryText = """select a,r from a=node(%A%) pattern a-[r?:LOVES]->()""",
      returns = """A node, and +null+, since the node has no relationships.""",
  ...
  @Test def shortestPathBetweenTwoNodes() {
    testQuery(
      ...
      queryText = """select p from d=node(%D%), e=node(%E%) pattern p = shortestPath( d-[*..15]->e )""",
 ...
 like relationship type, max hops and direction are all used when finding the shortest path.""",
  ...
  @Test def introduceNamedPath() {
    testQuery(
      ...
      queryText = """select p from a=node(%A%) pattern p = a-->b""",
