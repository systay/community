...
class NamedPathPipeTest extends GraphDatabaseTestBase with Assertions {
  ...
  @Test def pathsAreExtractedCorrectly() {
    ...
    assert(pipe.createResults(QueryState.forTest()) === List(Map("a" -> a, "b" -> b, "x" -> p, "p" -> p)))
  }
  ...
  @Test def pathsAreTurnedRightSideAround() {
    ...
    assert(pipe.createResults(QueryState.forTest()) === List(Map("a" -> a, "b" -> b, "x" -> p, "p" -> PathImpl(a, r1, b, r2, c))))
  }
}