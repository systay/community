...
class SortPipeTest extends JUnitSuite {
  @Test def emptyInIsEmptyOut() {
    ...
    assertEquals(List(), sortPipe.createResults(QueryState.forTest()).toList)
  }
  ...
  @Test def simpleSortingIsSupported() {
    ...
    assertEquals(List(MutableMap("x" -> "A"), MutableMap("x" -> "B")), sortPipe.createResults(QueryState.forTest()).toList)
  }
  ...
  @Test def sortByTwoColumns() {
    ...
    assertEquals(List(
      MutableMap("x" -> "A", "y" -> 100),
      MutableMap("x" -> "B", "y" -> 10),
      MutableMap("x" -> "B", "y" -> 20)), sortPipe.createResults(QueryState.forTest()).toList)
  }
  ...
  @Test def sortByTwoColumnsWithOneDescending() {
    ...
    assertEquals(List(
      MutableMap("x" -> "A", "y" -> 100),
      MutableMap("x" -> "B", "y" -> 20),
      MutableMap("x" -> "B", "y" -> 10)), sortPipe.createResults(QueryState.forTest()).toList)
  }
  ...
  @Test def shouldHandleSortingWithNullValues() {
    ...
    assertEquals(List(
      MutableMap("y" -> 1),
      MutableMap("y" -> 2),
      MutableMap("y" -> null)), sortPipe.createResults(QueryState.forTest()).toList)
  }
}
