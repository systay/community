...
trait Updates extends Base with Expressions with StartClause with Strings {
  def updates: Parser[Seq[UpdateAction]] = rep(delete | set | foreach | relate) ^^ (cmds => cmds.flatten)

  def foreach: Parser[Seq[UpdateAction]] = FOREACH ~> "(" ~> identity ~ IN ~ expression ~ ":" ~ opt(createStart) ~ opt(updates) <~ ")" ^^ {
  ...
  def delete: Parser[Seq[UpdateAction]] = DELETE ~> commaList(expression) ^^ {
  ...
  def set: Parser[Seq[UpdateAction]] = SET ~> commaList(propertySet)

  def relate: Parser[Seq[UpdateAction]] = RELATE ~> usePattern(translate) ^^ (links => Seq(RelateAction(links: _*)))
