...
class OrderByTest extends DocumentingTestBase {
  ...
  @Test def sortByName() {
    testQuery(
      ...
      queryText = """select n from n=node(%C%,%A%,%B%) order by n.name""",
  ...
  @Test def sortByNameReverse() {
    testQuery(
      ...
      queryText = """select n from n=node(%C%,%A%,%B%) order by n.name DESC""",
  ...
  @Test def order_by_nullable_property() {
    testQuery(
      ...
      queryText = """select n.length?, n from n=node(%C%,%A%,%B%) order by n.length?""",
