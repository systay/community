...
public class AclExampleTest extends AbstractJavaDocTestbase
{
    ...
    public void ACL_structures_in_graphs()
    {
        ...
        String query = "from root=node:node_auto_index(name = 'FileRoot') pattern root-[:contains*0..]->(parentDir)-[:leaf]->file select file";
        ...
        //Ownership
        query = "select file, user from root=node:node_auto_index(name = 'FileRoot') pattern root-[:contains*0..]->()-[:leaf]->file<-[:owns]-user";
        ...
        //ACL
        query = "SELECT file.name, dir.name, role.name, readUser.name " +
                "FROM file=node:node_auto_index('name:File*') " +
        		"PATTERN file<-[:leaf]-()<-[:contains*0..]-dir<-[?:canRead]-role-[:member]->readUser ";
